...
st.markdown("[Gitlab](https://gitlab.com/HP-SCDS/Observatorio/2023-2024/aiwantthejob/usc-aiwantthejob/)")
...
if st.session_state.page == "login":
    ...
    if login:
        ...
        if id is not None:
            ...
            st.session_state.grade_array = [1, 2]
            placeholder.empty()
...
if st.session_state.page == "chat":
    ...
    def do_query_test() -> None:
        ...
        output = "This is an interviewer question: given the context X, what would Y do?"
    ...
    def do_response_test() -> None:
        ...
        output, array = f"Grade: {grade}\n This evaluation was ...", st.session_state.grade_array + [grade]
    ...
    def do_dummy_response_test() -> None:
        ...
        st.session_state["input"] = ""
        output = "An example on how you would answer this would be ..."
        st.session_state["generated"].append(output)
    ...
    with col1:
        st.button("Ask for question", key="b_pregunta", on_click=send_query, args=["question"], disabled=st.session_state.get("disabled", True))
    
    with col2:
        st.button("Evaluate Answer", key="b_respuesta", on_click=send_query, args=["answer"], disabled=st.session_state.get("disabled", True))

    with col3:
        st.button("Ask for example", key="b_example", on_click=send_query, args=["example"], disabled=st.session_state.get("disabled", True))

    # Manego de la subida de documentos para el rag
    ...
    if st.session_state.grade_array:
        ...
        df = pd.DataFrame([st.session_state.grade_array, st.session_state.grade_array]).transpose()
        st.line_chart(df[1], use_container_width=True, height=300)
